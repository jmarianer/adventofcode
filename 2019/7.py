...
def do_the_thing(p):
    sig = 0
    for i in p:
        sig = list(runprog(prog, iter([i, sig])))[0]
    return sig
...
def do_thing2(p):
    ...
    queue1.put(p[0])
    queue1.put(0)
    prog1 = runprog(prog, queue_iterator(queue1))
    prog2 = runprog(prog, itertools.chain([p[1]], prog1))
    prog3 = runprog(prog, itertools.chain([p[2]], prog2))
    prog4 = runprog(prog, itertools.chain([p[3]], prog3))
    prog5 = runprog(prog, itertools.chain([p[4]], prog4))
    for i in prog5:
        sig = i
        queue1.put(i)
    return sig
